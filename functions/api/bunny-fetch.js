// ✅ NEW: براؤزر میں سیدھا یہ URL کھول کر چیک کریں کہ function deploy ہوئی ہے اور environment variables مل رہی ہیں یا نہیں
export async function onRequestGet(context) {
    const { env } = context;
    return new Response(JSON.stringify({
        message: 'bunny-fetch function چل رہی ہے ✅ (یہ صرف تشخیصی پیغام ہے، اصل کام POST request سے ہوتا ہے)',
        BUNNY_LIBRARY_ID_present: !!env.BUNNY_LIBRARY_ID,
        BUNNY_STREAM_API_KEY_present: !!env.BUNNY_STREAM_API_KEY
...
export async function onRequestPost(context) {
    ...
    try {
        // ✅ NEW: بعض سرورز صرف براؤزر جیسی request قبول کرتے ہیں (hotlink protection) —
        // اسی لیے Bunny کو بھی ایک عام User-Agent اور اصل ویڈیو کے اپنے domain جیسا Referer بھیجنے کو کہیں
        let refererHeader = '';
        try { refererHeader = new URL(sourceUrl).origin + '/'; } catch (e) { /* ignore */ }
