            ...
            <div style="display:grid; grid-template-columns:repeat(${reels.length}, minmax(120px, 160px)); gap:10px;">
                ${reels.map(v => `
                    <div class="reel-shelf-card" data-video-id="${v.id}" style="cursor:pointer; border-radius:10px; overflow:hidden; position:relative; aspect-ratio:9/16; background:#000;">
                        <img src="${getThumbnail(v.embed_code)}" style="width:100%; height:100%; object-fit:cover;" loading="lazy" alt="${v.title || 'Reel'}">
...
function renderAdminVideos() {
    ...
    tbody.innerHTML = state.videos.map(video => {
        ...
        // ✅ FIX: پہلے یہاں SVG کے اندر double-quotes تھے جو img کے src="..." کو غلطی سے وقت سے پہلے بند کر دیتے تھے، پورا HTML ٹوٹ جاتا تھا
        ...
        for (let i = 0; i < toSave.length; i++) {
            ...
            try {
                ...
                if (e.message && e.message.includes('no unique or exclusion constraint')) {
                    try {
                        const { error: fallbackError } = await supabase.from('videos').insert(v);
