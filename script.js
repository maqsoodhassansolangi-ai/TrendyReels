...
await supabase.post('videos', { 
    ...
    published: true,   // ← یہاں کوما لگا دیں
description: autoGenerateDescription(title),
tags: autoGenerateTags(title, suggestedCategory)
});
        await loadVideos();
        return true;
    ...
    try {
        const { data, error } = await supabase
            ...
            .upsert({ 
                ...
                is_copyright_free: isCopyrightFree,
                published: true,
thumbnail: thumbnail,   // ← یہاں کوما لگا دیں
description: autoGenerateDescription(title),
tags: autoGenerateTags(title, assignedCategory)
            }, { onConflict: 'url' });
...
function showReviewPanel(videos) {
    ...
    document.getElementById('reviewPanelSave').onclick = async () => {
        ...
        document.querySelectorAll('.review-item').forEach(item => {
            ...
            if (isApproved) {
                const v = videos[idx];
                toSave.push({ ...v, category: category, description: autoGenerateDescription(v.title), tags: autoGenerateTags(v.title, category) });
            }
        });
