// ============================================
// TrendyReels - V3.4.1 ULTIMATE - PART 1
// ============================================
...
// ============================================
// TrendyReels - V3.4.1 ULTIMATE - PART 2
// ============================================
...
// ============================================
// TrendyReels - V3.4.1 ULTIMATE - PART 3
// ============================================
...
async function ensureCategory(categoryName) {
    ...
    // 1. سب سے پہلے چیک کریں کہ کیا یہ کیٹگری پہلے سے موجود ہے؟
    ...
    // 2. اگر موجود ہے تو وہی واپس کر دیں
    if (existing) return existing.name;

    // 3. اگر موجود نہیں ہے تو نئی کیٹگری بنا دیں
...
// --- THE ULTIMATE SMART PARSER (FULLY UPGRADED WITH CATEGORY MANAGER) ---
async function processVideoData(videoData) {
    // Step 1: Find the URL/Link/EmbedCode from ANY column name
    let url = null;
    let title = null;
    let category = null;
    ...
    for (const key of possibleUrlKeys) {
        if (videoData[key]) { url = videoData[key]; break; }
    }
    for (const key of possibleTitleKeys) {
        if (videoData[key]) { title = videoData[key]; break; }
    }
    for (const key of possibleCategoryKeys) {
        if (videoData[key]) { category = videoData[key]; break; }
    }

    // Step 2: Auto-Assign Category if missing (using keyword detection)
    if (!category && title) {
        const keywords = {
            'cricket': 'Cricket', 'news': 'News', 'islamic': 'Islamic',
            'technology': 'Technology', 'tech': 'Technology',
            'comedy': 'Comedy', 'funny': 'Comedy', 'sports': 'Sports',
            'education': 'Education', 'fitness': 'Fitness-Fashion', 'fashion': 'Fitness-Fashion'
        };
    ...
    // Step 3: Ensure the category exists in Supabase (Create if missing)
    const assignedCategory = await ensureCategory(category || 'General');

    // Step 4: Auto-Detect Embed Code or URL
    let embedCode = '';
    let finalUrl = '';
    ...
    // Step 5: Auto-Extract Thumbnail (NEW)
    let thumbnail = '';
    ...
    // Step 6: Auto-Assign Copyright (NEW)
    ...
    if (title) {
        ...
        for (const kw of freeKeywords) {
            if (t.includes(kw)) isCopyrightFree = true;
        }
    }
...
async function init() {
    ...
    console.log('TrendyReels V3.4.1 ULTIMATE initialized!');
}
