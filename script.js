...
// ✅ Cleaner Tool کے لیے مشترکہ field-extraction لاجک — main Bulk Upload جیسا ہی مضبوط
function extractCleanerFields(row) {
    let url = '', title = '', category = 'General';
    for (const [rawKey, value] of Object.entries(row)) {
        const key = rawKey.replace(/^\uFEFF/, '').trim().toLowerCase();
        if (value === null || value === undefined || value === '') continue;
        const val = String(value).trim();
        if (!url && (val.replace(/\s/g, '').startsWith('http') || val.startsWith('<iframe') || val.startsWith('<video'))) url = val;
        if (!title && /title|(?:video[_\s-]?)?name|heading/.test(key) && !/tag|hashtag/.test(key)) title = val;
        if (/categor|genre|niche/.test(key)) category = val;
    }
    if (!url) {
        // ✅ اگر کسی مخصوص کالم نام سے نہ ملے تو ہر ویلیو میں تلاش کریں (embed code یا سادہ لنک)
        for (const value of Object.values(row)) {
            const val = String(value || '').trim();
            if (val.replace(/\s/g, '').startsWith('http') || val.startsWith('<iframe') || val.startsWith('<video')) { url = val; break; }
        }
    }
    return { title: title || 'Untitled Video', url, category: category || 'General' };
}
...
async function processVideoData(videoData, dryRun = false) {
    let url = null, title = null, category = null, description = null, tags = null;
    for (const [rawKey, value] of Object.entries(videoData)) {
        // ✅ CSV ہیڈرز میں بعض اوقات چھپا ہوا BOM کریکٹر یا اضافی خالی جگہ ہوتی ہے جو میچنگ ناکام بنا دیتی ہے — صاف کر لیں
        const key = rawKey.replace(/^\uFEFF/, '').trim();
        const keyLower = key.toLowerCase();
        if (value === null || value === undefined || value === '') continue;
        const valStr = typeof value === 'string' ? value.trim() : value;

        if (typeof valStr === 'string' && valStr.replace(/\s/g, '').startsWith('http')) {
            url = valStr;
        }
        // ✅ Title: 'title', 'name', 'video name', 'heading' جیسے کالم پکڑیں (مگر hashtag/tag سے نہ ٹکرائیں)
        if (!title && /title|(?:video[_\s-]?)?name|heading/.test(keyLower) && !/tag|hashtag/.test(keyLower)) {
            title = valStr;
        }
        if (!category && /categor|genre|niche/.test(keyLower)) {
            category = valStr;
        }
        // ✅ NEW: caption/description کالم بھی پکڑیں
        if (!description && /caption|description|desc\b|summary/.test(keyLower)) {
            description = valStr;
        }
        // ✅ NEW: hashtags/SEO tags/keywords کالم بھی پکڑیں
        if (!tags && /hashtag|seo[_\s-]?tags?|keywords?|tags?\b/.test(keyLower)) {
            tags = valStr;
        }
    }
    if (!url) {
