...
// ============================================
// ✅ NEW: SAVE TO WATCH LATER (Device-based — Like بٹن جیسا ہی طریقہ، localStorage میں محفوظ)
// ============================================
function getWatchLaterIds() {
    try { return JSON.parse(localStorage.getItem('tr_watch_later') || '[]'); } catch (e) { return []; }
}

function setupWatchLaterButton(video) {
    const btn = document.getElementById('watchLaterBtn');
    if (!btn) return;
    let savedIds = getWatchLaterIds();
    let isSaved = savedIds.includes(video.id);
    btn.textContent = isSaved ? '✅ Saved' : '🔖 Watch Later';
    btn.style.background = isSaved ? 'var(--primary-red)' : 'transparent';
    btn.style.color = isSaved ? 'white' : 'inherit';

    btn.addEventListener('click', () => {
        savedIds = getWatchLaterIds();
        isSaved = savedIds.includes(video.id);
        if (isSaved) {
            savedIds = savedIds.filter(id => id !== video.id);
            btn.textContent = '🔖 Watch Later';
            btn.style.background = 'transparent';
            btn.style.color = 'inherit';
        } else {
            savedIds.unshift(video.id); // ✅ نئی سیو شدہ ویڈیو سب سے اوپر آئے
            btn.textContent = '✅ Saved';
            btn.style.background = 'var(--primary-red)';
            btn.style.color = 'white';
        }
        localStorage.setItem('tr_watch_later', JSON.stringify(savedIds));
        renderWatchLaterSection();
    });
}

// ✅ Watch Later سیکشن کے کسی کارڈ پر ✕ دبانے سے سیدھا ہٹا دیں (بغیر ویڈیو کھولے)
function removeFromWatchLater(videoId) {
    let savedIds = getWatchLaterIds();
    savedIds = savedIds.filter(id => id !== videoId);
    localStorage.setItem('tr_watch_later', JSON.stringify(savedIds));
    renderWatchLaterSection();
    // ✅ اگر یہی ویڈیو اسی وقت modal میں کھلی ہو تو اس کا بٹن بھی واپس اپڈیٹ کر دیں
    const btn = document.getElementById('watchLaterBtn');
    if (btn && state.currentVideo && state.currentVideo.id === videoId) {
        btn.textContent = '🔖 Watch Later';
        btn.style.background = 'transparent';
        btn.style.color = 'inherit';
    }
...
function renderWatchLaterSection() {
    ...
    const savedIds = getWatchLaterIds();
    if (savedIds.length === 0) {
        section.style.display = 'none';
        return;
    }
    // ✅ state.videos ابھی تک لوڈ نہ ہوا ہو تو کچھ نہ کریں — videos لوڈ ہونے پر یہ فنکشن دوبارہ کال ہوگا
    const items = savedIds.map(id => state.videos.find(v => v.id === id)).filter(Boolean);
    if (items.length === 0) { section.style.display = 'none'; return; }

    section.style.display = 'block';
    grid.innerHTML = items.map(v => `
        <div class="history-item" style="position:relative;">
            <button onclick="event.stopPropagation(); removeFromWatchLater(${v.id})" title="ہٹائیں" style="position:absolute; top:4px; right:4px; width:22px; height:22px; border-radius:50%; border:none; background:rgba(0,0,0,0.6); color:white; font-size:12px; cursor:pointer; z-index:2; line-height:1;">✕</button>
            <img class="history-thumb" src="${getThumbnail(v.embed_code) || 'data:image/svg+xml,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'160\' height=\'90\'><rect fill=\'%23ccc\'/></svg>'}" alt="${v.title}" onclick="openModernVideoModal(state.videos.find(x => x.id === ${v.id}))">
            <div class="history-title" onclick="openModernVideoModal(state.videos.find(x => x.id === ${v.id}))">${v.title || 'Untitled'}</div>
        </div>
    `).join('');
}
...
function activateReelSlide(slideEl) {
    ...
    if (provider.type === 'native') {
        ...
        attachHlsIfNeeded(media); // ✅ NEW: اگر یہ Bunny/HLS اسٹریم ہو تو hls.js سے چلائیں
...
function setupReelsObserver(startVideoId = null) {
    ...
    // ✅ اگر مین گرڈ سے کسی مخصوص Reel پر کلک کیا گیا ہو تو سیدھا اسی پر پہنچائیں
    let startSlide = slides[0];
    if (startVideoId) {
        const targetSlide = container.querySelector(`.reel-slide[data-video-id="${startVideoId}"]`);
        if (targetSlide) startSlide = targetSlide;
    }
    if (startSlide) {
        startSlide.scrollIntoView({ behavior: 'instant', block: 'start' });
        startSlide.classList.add('reel-active');
...
// ============================================
// ✅ NEW: PLAYLISTS (Public Site) — Admin کی بنائی ہوئی Playlists ہوم پیج پر دکھانا اور کھولنا
// ============================================
function renderPlaylistsSection() {
    const section = document.getElementById('playlistsSection');
    const grid = document.getElementById('playlistsGrid');
    if (!section || !grid) return;
    // ✅ صرف وہ Playlists دکھائیں جن میں کم از کم ایک ویڈیو موجود ہو
    const nonEmpty = (state.playlists || []).filter(pl => getPlaylistVideoIds(pl.id).length > 0);
    if (nonEmpty.length === 0) { section.style.display = 'none'; return; }

    section.style.display = 'block';
    grid.innerHTML = nonEmpty.map(pl => {
        const videoIds = getPlaylistVideoIds(pl.id);
        const firstVideo = state.videos.find(v => videoIds.includes(v.id));
        const cover = firstVideo ? (firstVideo.thumbnail || getThumbnail(firstVideo.embed_code)) : '';
        return `
            <div class="playlist-card" data-id="${pl.id}">
                <img class="playlist-cover" loading="lazy" src="${cover || 'data:image/svg+xml,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'160\' height=\'90\'><rect fill=\'%23ccc\'/></svg>'}" alt="${pl.name}">
                <div class="playlist-card-overlay">🎞️ ${videoIds.length}</div>
    ...
    grid.querySelectorAll('.playlist-card').forEach(card => {
        card.addEventListener('click', () => openPlaylistView(parseInt(card.dataset.id)));
    });
}

// ✅ کسی مخصوص Playlist کی تمام ویڈیوز/Reels ایک فل سکرین گرڈ میں دکھائیں
function openPlaylistView(playlistId) {
    const pl = (state.playlists || []).find(p => p.id === playlistId);
    if (!pl) return;
    const videoIds = getPlaylistVideoIds(playlistId);
    const videos = videoIds.map(id => state.videos.find(v => v.id === id)).filter(Boolean);

    const overlay = document.getElementById('playlistViewOverlay');
    const titleEl = document.getElementById('playlistViewTitle');
    const grid = document.getElementById('playlistViewGrid');
    if (!overlay || !titleEl || !grid) return;
    titleEl.textContent = `🎞️ ${pl.name} (${videos.length})`;

    if (videos.length === 0) {
        grid.innerHTML = `<div class="empty-state"><p>اس Playlist میں ابھی کوئی ویڈیو نہیں۔</p></div>`;
    } else {
        // ✅ مین گرڈ کی طرح ہی کارڈز — تھمب نیل فوری (poster/getThumbnail کے ذریعے، بغیر بفرنگ کی تاخیر)
        grid.innerHTML = videos.map(video => {
            const posterUrl = video.thumbnail || getThumbnail(video.embed_code);
            const mediaHtml = `<img class="video-thumbnail" src="${posterUrl}" alt="${video.title || 'Video'}" loading="lazy" onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%22320%22 height=%22180%22><rect fill=%22%23CCCCCC%22 width=%22320%22 height=%22180%22/></svg>'">`;
            return `<div class="video-card" data-id="${video.id}">${mediaHtml}<div class="video-info"><div class="video-title">${video.title || 'Untitled Video'}</div><div class="video-meta"><span class="video-category">${video.category || 'Uncategorized'}</span>${video.is_reel ? '<span class="copyright-badge">🎬 Reel</span>' : ''}</div></div></div>`;
        }).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    const closeBtn = document.getElementById('closePlaylistViewBtn');
    if (closeBtn) {
        closeBtn.addEventListener('click', () => {
            document.getElementById('playlistViewOverlay').classList.remove('active');
        });
    }
});
...
function openModernVideoModal(video) {
    ...
    player.innerHTML = `
        ...
        <div style="display:flex; justify-content:space-between; margin-top:10px; flex-wrap:wrap; gap:6px;">
            ...
            <div style="display:flex; gap:6px; flex-wrap:wrap;">
                ...
                <button id="watchLaterBtn" class="action-btn" style="background:transparent; border:1px solid #666; color:inherit;">🔖 Watch Later</button>
    ...
    // ✅ NEW: Watch Later بٹن — اسی device پر یاد رہتا ہے
    setupWatchLaterButton(video);
    ...
    if (provider.type === 'native') {
        ...
        attachHlsIfNeeded(videoEl); // ✅ NEW: اگر یہ Bunny/HLS اسٹریم ہو تو hls.js سے چلائیں
